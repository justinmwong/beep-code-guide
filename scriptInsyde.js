...
const getAirtableData = async () => {
  ...
  try {
    const insydeResponse = await fetch(
      "/* Insert API here */",
      options
    );
    const insydeData = await insydeResponse.json();
    console.log(insydeData);

    let insydeContent = "";
    const records = insydeData.records || [];
    ...
    let insyde = document.getElementById("insyde");
    if (insyde) {
      insyde.innerHTML = insydeContent;
      ...
      console.error("Element with id 'insyde' not found.");
    }
  } catch (error) {
